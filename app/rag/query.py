...
class QueryInterpreter:
    ...
    REFERENTIAL_MARKERS = (
        "그거", "그건", "그것", "그것의", "그중", "그 yaml", "그 코드", "그 예시", "그럼", "다시", "바꿔", "로도",
        "that", "this", "those", "it", "its", "again", "also",
    )
    ...
    DOCUMENT_QUERY_HINTS = (
        "설명", "정리", "비교", "차이", "예시", "코드", "yaml", "문서", "페이지", "출처", "무엇", "뭐", "왜",
        "어떻게", "알려줘", "보여줘", "what", "how", "why", "compare", "difference", "explain",
    )
    ...
    def interpret(self, user_message: str, query_result: dict | None = None, topic_state: dict | None = None) -> QueryInterpretation:
        ...
        return QueryInterpretation(intent, is_document_query, resources, actions, format_constraints, response_shape, normalized_keywords, needs_multiturn_state)
    ...
    def _extract_resources(self, normalized_message: str, normalized_keywords: list[str], topic_state: dict) -> list[str]:
        ...
        if not resources and self._should_inherit_resources(normalized_message, normalized_keywords, topic_state):
            ...
            if last_code_resource_kind in known_resources and last_code_resource_kind not in resources:
                resources.append(last_code_resource_kind)
        return resources

    def _should_inherit_resources(self, normalized_message: str, normalized_keywords: list[str], topic_state: dict) -> bool:
        if not (
        ...
        if len(normalized_message) <= 32 and topic_state.get("active_topic"):
            return True
        return bool({"yaml", "yml", "manifest", "code", "example", "sample", "demo", "예시", "코드"} & set(normalized_keywords))
    ...
    def _determine_response_shape(self, normalized_message: str, format_constraints: list[str], actions: list[str]) -> str:
