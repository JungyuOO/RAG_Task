...
class ChatTurnOrchestrator:
    ...
    async def run(
        ...
        current_topic_id_before = (
            str(topic_state_before.get("last_active_topic_id") or "")
            if isinstance(topic_state_before, dict)
            else ""
        )
        session_topics_before = deps.session_repository.list_topics(session_id)
        procedure_followup = None
        if not deps.should_skip_procedure_shortcut(
            user_message,
            session_topics_before,
            current_topic_id_before or None,
        ):
        ...
        if procedure_followup:
            built = deps.build_procedure_followup_answer(
                procedure_followup,
                topic_state_before.get("procedure_state", {}),
            )
            if built is not None:
                ...
                resolved_topic_id = (
                    topic_state_before.get("last_active_topic_id")
                    if isinstance(topic_state_before, dict)
                    else None
                )
                deps.store_assistant_turn(
                    session_id,
                    procedure_answer,
                    final_payload,
                    resolved_topic_id,
        ...
        if append_user_turn:
            ...
            resolved_topic_id = deps.ensure_topic_for_resolution(
                session_id,
                user_message,
                state,
                user_turn_id,
        ...
        cache_key = deps.build_answer_cache_key(
            ...
            rewritten_query=deps.canonical_cache_query(
                user_message,
                rewritten_query,
        ...
        if cached_answer is not None:
            full_text = deps.answer_service.sanitize_answer(
            ...
            final_answer, answer_citations, final_payload = deps.finalize_answer(
                streamed.strip(),
                rewritten_query,
                use_retrieved_context,
                top_score,
        ...
        if use_retrieved_context and context_blocks and deps.should_run_judge_agent(
            policy_decision,
            query_interpretation,
            top_score,
        ):
    ...
    async def _handle_terminal_policy_answers(self, **kwargs):
        ...
        if policy_decision.turn_type == "conversational_ack":
            answer = deps.build_policy_answer("conversational_ack", top_score)
            return _emit(answer, "conversational", False)
        if policy_decision.turn_type == "greeting":
            answer = deps.build_policy_answer("greeting", top_score)
            return _emit(answer, "greeting", False)
        if policy_decision.turn_type == "general_chat":
            answer = deps.build_policy_answer("general_chat", top_score)
            return _emit(answer, "general", False)
        if policy_decision.turn_type == "document_query" and not kwargs["use_retrieved_context"]:
            answer = deps.build_policy_answer("document_query", top_score)
            return _emit(answer, "general", True)
        return None
