...
export function useOcpConnection(): OcpConnectionController {
  ...
  async function createConnection(request: OcpConnectionRequest) {
    return connectWithRequest(request, "연결 프로필을 생성하는 중입니다.");
  }

  async function connectSavedProfile(savedProfileId: string) {
    const savedProfile = savedProfiles.find((item) => item.id === savedProfileId);
    if (!savedProfile) {
  ...
  function removeSavedProfile(savedProfileId: string) {
    const removedProfile = savedProfiles.find((item) => item.id === savedProfileId);
