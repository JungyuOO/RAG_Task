...
class _HtmlPageContentParser(HTMLParser):
    IGNORE_TAGS = {"script", "style", "nav", "header", "footer"}
    TEXT_TAGS = {"p", "li", "pre", "code"}

    def __init__(self) -> None:
        super().__init__(convert_charrefs=True)
        self.sections: list[_HtmlPageSection] = []
        self.page_title = ""
        self._ignored_depth = 0
        self._content_depth = 0
        self._seen_content_root = False
        ...
        self._current_section: _HtmlPageSection | None = None
    ...
    def _in_content(self) -> bool:
        ...
        if self._content_depth > 0:
            return True
        return self._inside_body and not self._seen_content_root
    ...
    def _start_section(self, *, level: int, title: str, anchor: str) -> None:
        ...
        self._current_section = _HtmlPageSection(level=level, title=title, anchor=anchor, path=path)
    ...
    def parse(self, source: SourceDescriptor) -> ParsedDocumentBundle:
        ...
        bundle = self.build_empty_bundle(
            ...
            metadata={
                ...
                "expected_specialization": "general article/page html",
            ...
            for block_index, (tag, text) in enumerate(section.blocks, start=1):
                ...
                blocks.append(
                    ParsedBlock(
                        ...
                        section_title=section.title,
                        section_path=section_path,
                        resource_hints=_detect_resource_hints(section.title, " ".join(section_path), text),
